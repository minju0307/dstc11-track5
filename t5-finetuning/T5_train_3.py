...
def T5Trainer(dataframe, test_dataframe, source_text, target_text, model_params, output_dir="./outputs/" ):
    ...
    special_tokens_dict = {'additional_special_tokens': ['[SEP]']} ## knowledge와 dialogue context를 구분해주는 구분자
    ...
    model.resize_token_embeddings(len(tokenizer)) ## 모델 resize 해줘야 한다
    ...
    for epoch in range(model_params["TRAIN_EPOCHS"]):
        ...
        predictions, actuals = validate(tokenizer, model, device, val_loader, model_params["MAX_TARGET_TEXT_LENGTH"]) ## 지금은 오로지 train 만 할 것이기 때문에
    ...
    if model_params["TRAIN_EPOCHS"] ==0: ## for few shot learning
        # evaluating test dataset
...
if __name__=='__main__':
    ...
    wandb.login()
    wandb.init(project='clean_train_chatgpt_aug_t5_finetuning')
    ...
    #load dataset
    df1 = pd.read_csv(f"dataset/chatgpt_aug_finetuning.csv", sep=',')
    df3 = pd.read_csv(f"dataset/original_train.csv", sep=',')

    df= df1 + df3


    #load test dataset
    df2 = pd.read_csv(f"dataset/original_test.csv", sep=',')
    ...
    output_dir='clean_train_chatgpt_aug'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    T5Trainer(dataframe=df, test_dataframe=df2, source_text="src_text", target_text="trg_text", model_params=model_params, output_dir=output_dir)