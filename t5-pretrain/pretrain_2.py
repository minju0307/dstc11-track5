...
from T5Dataset_2 import T5Dataset, DATASET_DIR, Parts
...
def main(
        t5_model: str = "google/flan-t5-xl", lr: float = 1e-4,  # 3^4
        epochs: int = 1, fp16: bool = False,
        dataset: Corpus = Corpus.T5, batch_size: int = 4,
        max_len: int = 256, grad_accu: int = 1,
        num_gpus: int = 6
):
    ...
    trainer = pl.Trainer(
        ...
        logger=[
            ...
            pl.loggers.WandbLogger(project="dstc-t5-pretrain-last-response-more")
        ],
    ...
    pl_module.model.save_pretrained(DATASET_DIR / f"{config.base_t5_model}")
    pl_module.tokenizer.save_pretrained(DATASET_DIR / f"{config.base_t5_model}")
